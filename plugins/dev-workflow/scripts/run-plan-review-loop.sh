...
# Args: [--force] [<plan-path>] — the first non-flag arg is the plan path
PLAN="doc/current_plan.md"
for a in "$@"; do [[ "$a" != --* ]] && PLAN="$a"; done

# (a) require the plan file to exist
...
# (b) build the plan manifest
"$ROOT/scripts/detect-review-type.sh" plan "$PLAN" --force
...
echo "Ralph-loop initialized (mode: plan-review). Begin iteration 1."
